'use client';
import Image from 'next/legacy/image';
import Button from '../Common/Button';
import SectionTitle from '../Common/SectionTitle';
import style from './TabServices.module.css'
const TabsIndex__item4 = ({onClickBtn}) => {
	return (
		<>
			<div className={`md:flex md:gap-[30px] lg:gap-[60px] md:flex-row-reverse`}>
				<div className="md:basis-1/2">
					<div className='img__fixed-height'>
						<Image
							src='/images/services/tab-img04.jpg'
							layout='fill'
							objectFit='cover'
							quality={0}
							loading={`lazy`}
							alt='100% Satisfaction Guarantee'
						/>
					</div>
				</div>
				<div className="md:basis-1/2 only-md:mt-[40px] sm-max:mt-[35px]">
					<SectionTitle
						dataTitle="Reconstruction Services"
						dataAddClass="blocktitle__topNone2"
					/>
					<p>
					Complete reconstruction and rebuild services to restore your property after damage. We handle everything from structural repairs to finishing work, ensuring your property is returned to pre-loss condition or better. Our team works with you every step of the way.
					</p>
					<ul className='list__type1 list__top'>
						<li>Structural repairs and rebuilding</li>
						<li>Drywall, flooring, and finish work</li>
						<li>Complete project management</li>
						<li>Quality craftsmanship guaranteed</li>
					</ul>
					<Button
						className={`${style.btn__top}`}
						onClick={(e) => {
							onClickBtn();
						}}
					>
						Schedule Service
					</Button>
				</div>
			</div>
		</>
	)
};
export default TabsIndex__item4;